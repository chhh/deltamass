...
public class KDEUtils {
   ...
   * @param nearestPeak near which mass will we be searching for a single good peak to estimate
   * bandwidth
   * @param maxDistance the max distance from the {@code nearestPeak} to accept a point for the
   * estimate (this will be used as +/- distance)
   * @return estimated bandwidth
   */
  public static double estimateBandwidth(DenseVector massDiffs, double nearestPeak,
      double maxDistance) {
    ...
    double estimatedBandwidth = KernelDensityEstimator.BandwithGuassEstimate(acceptedVec);
