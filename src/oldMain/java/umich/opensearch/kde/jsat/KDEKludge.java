package umich.opensearch.kde.jsat;
...
import umich.opensearch.kde.params.kernels.KernelFuncPrime2;
 ...
 */
public class KDEKludge extends ContinuousDistribution {
  /*
   * README
   * Implementation note:
   ...
   */
  protected double h;
  protected double Xmean, Xvar, Xskew;

  protected KernelFunction k;
  ...
  public KDEKludge(Vec dataPoints) {
    this(dataPoints, autoKernel(dataPoints));
  }

  public KDEKludge(Vec dataPoints, KernelFunction k) {
    this(dataPoints, k, BandwithGuassEstimate(dataPoints));
  }

  public KDEKludge(Vec dataPoints, KernelFunction k, double[] weights) {
    this(dataPoints, k, BandwithGuassEstimate(dataPoints), weights);
  }

  public KDEKludge(Vec dataPoints, KernelFunction k, double h) {
    setUpX(dataPoints);
    this.k = k;
    this.h = h;
  }

  public KDEKludge(Vec dataPoints, KernelFunction k, double h, double[] weights) {
   ...
   * Copy constructor
   */
  private KDEKludge(double[] X, double h, double Xmean, double Xvar, double Xskew, KernelFunction k,
      double sumOfWeights, double[] weights) {
  ...
  /**
   * Automatically selects a good Kernel function for the data set that balances Execution time and
   * accuracy
   *
  ...
  public void setKernelFunction(KernelFunction k) {
    this.k = k;
  }
  ...
  private void setUpX(Vec S, double[] weights) {
    ...
    List<ProbailityMatch<Double>> sorter = new ArrayList<>(S.length());
  ...
  /**
   * By the point you can call this, the array should have already been sorted.
   */
  public double[] getX() {
    return X;
  }
  ...
  /**
   * Calculate PDF using the provided value of bandwidth.
   *
   * @param x the point to calculate at
   * @param h custom bandwidth value
   */
  public double pdf(double x, double h) {
    return pdf(x, -1, h);
  }
   ...
   * @param j the sorted index of the value to leave. If a negative value is given, the PDF with all
   * values is returned
   * @param h which bandwidth to use
   * @return the pdf with the given index left out
   */
  private double pdf(double x, int j, double h) {
    /*
     *              n
     *            =====  /x - x \
     *         1  \      |     i|
  ...
  public double pdfPrime2(double x) {
    return pdfPrime2(x, -1, h);
  }

  public double pdfPrime2(double x, double h) {
    return pdfPrime2(x, -1, h);
  }

  protected double pdfPrime2(double x, int j, double h) {
    /*
     *              n
     *            =====  /x - x \
     *         1  \      |     i|
    ...
    if (!(k instanceof KernelFuncPrime2)) {
      throw new IllegalStateException(
          "You can only call 2nd derivative of PDF if you've set a kernel function that " +
              "is an instance of KernelFuncPrime2");
    }
    KernelFuncPrime2 kernelFuncPrime2 = (KernelFuncPrime2) k;
    ...
    for (int i = Math.max(0, from); i < Math.min(X.length, to + 1); i++) {
      if (i != j) {
        sum += kernelFuncPrime2.kPrime2((x - X[i]) / h) * getWeight(i);
      }
    }
  ...
  public double pdfPrime(double x) {
    return pdfPrime(x, -1, h);
  }

  public double pdfPrime(double x, double h) {
    return pdfPrime(x, -1, h);
  }

  protected double pdfPrime(double x, int j, double h) {
    /*
     *              n
     *            =====  /x - x \
     *         1  \      |     i|
    ...
    for (int i = Math.max(0, from); i < Math.min(X.length, to + 1); i++) {
      if (i != j) {
        sum += k.kPrime((x - X[i]) / h) * getWeight(i);
      }
    }
  ...
  /**
   * Sets the bandwidth used for smoothing. Higher values make the pdf smoother, but can obscure
   * features. Too small a bandwidth will causes spikes at only the data points.
   *
  ...
  @Override
  public KDEKludge clone() {
    return new KDEKludge(X, h, Xmean, Xvar, Xskew, k, sumOFWeights, weights);
  }
  ...
  public boolean equals(Object obj) {
    ...
    if (!(obj instanceof KDEKludge)) {
      return false;
    }
    KDEKludge other = (KDEKludge) obj;
